weather = ['HT', 'HT', 'Sunny', 'Storm', 'Sunny', 'HT', 'Storm', 'Sunny', 'HT', 'HT', 'Storm', 'HT', 'Sunny', 'HT', 'HT', 'HT', 'Storm', 'Storm', 'HT', 'HT', 'Sunny', 'Sunny', 'HT', 'Sunny', 'Storm', 'HT', 'Sunny', 'Sunny', 'HT', 'HT']
E = {
    1: [2, 25],
    2: [3],
    3: [2, 4, 25],
...
food_range = lambda w: range((load_limit - w * quality[0]) // quality[1])

for d in range(T+1):
    for p in range(1, N+1):
        for w in water_range:
            for f in food_range(w):
                DP[(d,p,w,f)] = (float('-inf'), (), '', '')

for w in water_range:
    for f in food_range(w):
        DP[(0,1,w,f)] = (initial_money - price[0] * w - price[1] * f, (), '', '')
        ...
        if p == V:
            for w in water_range:
                for f in food_range(w):
                    ...
                    fb_range = lambda w: range((capacity - w * quality[0]) // quality[1])
                    for wb in wb_range:
